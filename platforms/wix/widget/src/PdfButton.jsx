...
const PdfButton = ({ config }) => {
  ...
  const generatePdf = async () => {
    ...
    try {
      ...
      if (result.jobId) {
        const pdf = await pollForJobCompletion(backendUrl, result.jobId);
  ...
  const pollForJobCompletion = async (backendUrl, jobId, maxAttempts = 60) => {
    for (let i = 0; i < maxAttempts; i++) {
      await new Promise(resolve => setTimeout(resolve, 2000)); // Wait 2 seconds

      const statusResponse = await fetch(`${backendUrl}/${jobId}`);
